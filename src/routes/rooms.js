...
router.post('/:id/join', async (req, res) => {
  try {
    const { avatar = '🦄', is_bot = false } = req.body;
    ...
    const { data: room } = await supabase
      .from('rooms').select('cfg_room_open, phase').eq('id', room_id).single();
    ...
    const { data: player, error } = await supabase
      .from('players')
      .insert({ room_id, anon_handle: handle, avatar, is_bot })
      .select()
      .single();

    if (error) throw error;

    // Emit to room via socket (injected via app.locals)
...
router.post('/:id/start', async (req, res) => {
  try {
    ...
    req.app.locals.io?.to(req.params.id).emit('game_started');
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
